...
const Sidebar = (props: Props) => {
  ...
  const links = [
    { name: "profile", icon: <CgProfile className="text-4xl sm:text-2xl" /> },
    { name: "password", icon: <MdPassword className="text-4xl sm:text-2xl" /> },
    {
      name: "orders",
      icon: <FaClipboardList className="text-4xl sm:text-2xl" />,
    },
    { name: "reviews", icon: <MdReviews className="text-4xl sm:text-2xl" /> },
    {
      name: "favorites",
      icon: <AiFillHeart className="text-4xl sm:text-2xl" />,
    },
  ];
  ...
  return (
    <div className="sticky flex flex-col justify-evenly h-full w-[50px] sm:w-[20%] bg-slate-100 dark:bg-primary-500 rounded-lg">
      {isAboveMediumScreens ? (
        links.map((e, index) => (
          <Link
            href={`/profile/${e.name === "profile" ? "" : e.name}`}
            key={index}
          >
            <div
