import { allProductsMostSelling, cleanUpProductsMostSelling, getAllProductsMostSelling, selectAllProductsMostSellingStatus } from "@/state/products/products/productsSlice"
...
const MostSelling = (props: Props) => {
  const dispatch = useAppDispatch()
  const router = useRouter()
  const productsStatus = useSelector(selectAllProductsMostSellingStatus)
  const products = useSelector(allProductsMostSelling)
    ...
    (async () => {
      if (router.isReady) {
        if (productsStatus === EStateGeneric.IDLE) {
          await dispatch(getAllProductsMostSelling());
        }
      }
    })()
    return () => {
      dispatch(cleanUpProductsMostSelling())
    }
  }, [router.isReady])
  return (
    <div className={`w-[99vw] ${products.length ? '' : 'h-[50vh]'}`}>
      {productsStatus === EStateGeneric.SUCCEEDED && <div className='w-full'>
        <h2 className='text-xl font-semibold m-4 capitalize '>Top-selling products just for you!</h2>
        <Swiper
