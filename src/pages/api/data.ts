...
export default async function handler(
  ...
  const { method } = req;
  const { brandsQ, categoriesQ, countriesQ, subcategoriesQ, productsQ } =
    req.query;
  switch (method) {
    case "GET":
      try {
    ...
    case "POST":
      try {
        ...
        if (
          ...
          !countriesLength.length &&
          !subcategoriesLength.length &&
        ...
        return res
          .status(400)
          .json({ success: false, message: "the data exists" });
      } catch (error) {
        return res.status(400).json({ success: false, error });
      }
    case "DELETE":
